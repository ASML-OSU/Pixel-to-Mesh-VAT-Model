function patch_plot_v2(t,deformed_p,FID,componment,undeformed_p)
...
%   deformation body coordinates
%   FID: figure ID for plot in one figure.
%   component: what componet of this body, see below options
%   undeformation: undeformed coordinates
...
[sorted_nodal_label,sorted_id] = sort(deformed_p(:,1)); % p should have unique label here.

p_updated_only_cordinates = deformed_p(sorted_id,2:4);

p_original_only_cordinates = undeformed_p(sorted_id,2:4);
%%
...
%     max(p_updated_only_cordinates(:,3))
    max(col)
    %
...
%     save t_cquad4.txt t_cquad4 -ascii
%     save p_updated_only_cordinates.txt p_updated_only_cordinates -ascii
