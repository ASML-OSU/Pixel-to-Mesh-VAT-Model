...
%             gridflag = 0;
%             if length(linebdf)>8
                gridkeywords = strtrim(linebdf(1:7));
%                 gridflag = 1;
%             end
            %
            if strcmp(gridkeywords,'GRID')
                ...
                if strfind(linebdf(41: z_end_string_length ),'-')
                  ...
                  node_num=node_num+1;
            elseif strcmp(gridkeywords,'GRID*')
                
                  bdf_node(node_num,1)=str2num(linebdf(9:16)); % node ID
                  
                  % cordx
                  
                  cordx_string = linebdf(41:56);
                   cordy_string = linebdf(57:end);
                  
                   % % next line for cordz
                   
                   linebdf=fgetl(fid100);currentFPI=ftell(fid100);
                    cordz_string = linebdf(9:end);
                    
                  bdf_node(node_num,2)   = str2num(cordx_string);
                   bdf_node(node_num,3)   = str2num(cordy_string);
                    bdf_node(node_num,4)   = str2num(cordz_string);
                %%
                node_num=node_num+1;
            end
            %
