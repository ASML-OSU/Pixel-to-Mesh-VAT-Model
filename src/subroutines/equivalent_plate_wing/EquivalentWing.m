...
stiffenerelementnumber=10;

stiffenernodesnumber=2*stiffenerelementnumber+1;

Xstep=[0.6];

for step_par=1:length(Xstep)
    
    xstep=Xstep(step_par);
    
    wingmodel;
    
    %% Geometry Coordinates
    % Stru.bdfname='squareplate4by4.bdf';
    % Stru.bdfname='SquarePlate8noded8by8.bdf';%%
    % Stru.bdfname='SquarePlate8noded10by10.bdf';
    
    Stru.bdfname='squareplate12by12.bdf';
    % Stru.bdfname='RecOneStrStiffener18by18.bdf';
    % Stru.bdfname='RecOneStrStiffener20by20.bdf';
    % Stru.bdfname='RecOneStrStiffener24by24.bdf';
    % Stru.bdfname='RecOneStrStiffener36by36.bdf';
    
    CurrentFilePath='Z:\paper\Stiffened Plate\StiffenedPlateCode';
    
    Stru.pathfile=[CurrentFilePath '\InputBDF'];
    ...
    Stru.thickness=ratioTW(1)*Stru.width; %factort*Stru.length;
...
%         PlyAngle=[theta -theta theta -theta -theta theta -theta theta];
        ...
        PlyAngleStiffener=zeros(1,length(PlyAnglePlate));%PlyAngle;%[theta theta theta theta theta theta theta theta];
        %% =====================================================================
        ...
        stress_ratio=0;
        sxx0=1e5*1;
        syy0=1E5*1; % positive for compression; negative for tensile
        ...
        for depthNO=1:length(depthratio)
            ...
            %% ------------Stiffener-----------------------
            for stiffno=[1 :8] % constistent with the number in the code: stiffener_Gauss_points;
                ...
                KGbeam=GeometryStiffnessCurvedBeam(FEM,ebar,Stiffener,StiffenerGaussPoint);
            ...
            switch SOL_flag
                ...
                case '102'
                    FEM.BCtype='CFFF-3'; %% Four simple supported sides
                    [ActiveDof,SideDof]=EssentialBCPlate5Dof(FEM);
                    Solver='vibration';
                ...
                case '105'
                    ...
                    buckLF(AngleNo,depthNO)=critical_loadfactor;
...
%                     totalweight(step_par)=stiffenerWeight;
%                     buckPara(step_par)=buckparameter;
%                     buckN(step_par)=buckLoad;
                case '103'
                    %% -----------Boundary Conditions-----------
                    FEM.BCtype='CFFF-3'; %% Four simple supported sides
                    [ActiveDof,SideDof]=EssentialBCPlate5Dof(FEM);
...
% surf(DepthRatio,TTheta,abs(buckparameter));
...
% figure;
% [DesignStep,DepthRatio]=meshgrid(depthratio,Xstep);
% surf(DesignStep,DepthRatio,abs(buckparameter));
% xlabel(['Ply orientation (degree)'],'FontSize',14);
% ylabel(['Stiffener depth ratio (h_s/b_s)'],'FontSize',14);
% zlabel(['Buckling parameter'],'FontSize',14);
