function [K,Kelem]=LinearStiffnessLaminatedPlate_VAT_v2_X_towpreg(Mat, Stru,FEM,Laminate, T0T1,center,physical_length,lamina_info)
...
for elem=1:FEM.numberElements
    ...
    for ee=1:size(GaussWeights,1)
        ...
        for layer = 1:size(T0T1,1)
            ...
            elem_lamina_info = lamina_info(elem,layer);

            t =  full(sum(lamina_info(elem,:)))*Laminate.layer_thickness;



            zk0 = full(sum(lamina_info(elem,1:layer)) - lamina_info(elem,layer))*Laminate.layer_thickness;
            zk  = full(sum(lamina_info(elem,1:layer)))*Laminate.layer_thickness;

            % % symmetric check
            zk0 = -t/2 + full(sum(lamina_info(elem,1:layer-1)))*Laminate.layer_thickness;
            zk = zk0 + full(lamina_info(elem,layer))*Laminate.layer_thickness;
            %%

            [AmatrixK,DmatrixK,AshearK,BmatrixK,QthetaK,ThermalExpCoeff]=...
                LaminatedComposite_VAT_full(Mat,Stru,Laminate,layer,theta,zk,zk0);
